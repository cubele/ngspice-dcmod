...
static int dynamic_gmin_fast(CKTcircuit *, GMRESarr *, long int, long int, int);
static int spice3_gmin(CKTcircuit *, long int, long int, int);
static int spice3_gmin_fast(CKTcircuit *, GMRESarr *, long int, long int, int);
static int new_gmin(CKTcircuit*, long int, long int, int);
static int new_gmin_fast(CKTcircuit*, GMRESarr *, long int, long int, int);
static int gillespie_src(CKTcircuit *, long int, long int, int);
static int gillespie_src_fast(CKTcircuit *, GMRESarr *, long int, long int, int);
static int spice3_src(CKTcircuit *, long int, long int, int);
static int spice3_src_fast(CKTcircuit *, GMRESarr *, long int, long int, int);

int
CKTop_fast (CKTcircuit *ckt, GMRESarr *arr, long int firstmode, long int continuemode,
       int iterlim)
{
    int converged;
    ...
    if (ckt->CKTnumGminSteps >= 1) {
        if (ckt->CKTnumGminSteps == 1) {
            ...
            if (cp_getvar("dyngmin", CP_BOOL, NULL, 0)) {
                converged = dynamic_gmin_fast(ckt, arr, firstmode, continuemode, iterlim);
            }
            /* first the old gmin, then the new gmin */
            else {
                converged = dynamic_gmin_fast(ckt, arr, firstmode, continuemode, iterlim);
                if(converged != 0) {
                    converged = new_gmin_fast(ckt, arr, firstmode, continuemode, iterlim);
                }

            }
        }
        else {
            converged = spice3_gmin_fast(ckt, arr, firstmode, continuemode, iterlim);
        }
    ...
    if (ckt->CKTnumSrcSteps >= 1) {
        if (ckt->CKTnumSrcSteps == 1)
            converged = gillespie_src_fast(ckt, arr, firstmode, continuemode, iterlim);
        else
            converged = spice3_src_fast(ckt, arr, firstmode, continuemode, iterlim);
        if (converged == 0) /* If gmin-stepping worked... move out */
            return converged;
    }
...
#define NIiter(ckt, lim) NIiter_fast(ckt, arr, lim)
static int
dynamic_gmin_fast (CKTcircuit *ckt, GMRESarr *arr, long int firstmode,
              long int continuemode, int iterlim)
{
    double OldGmin, gtarget, factor;
    int converged;
...
static int
new_gmin_fast(CKTcircuit* ckt, GMRESarr *arr, long int firstmode,
    long int continuemode, int iterlim)
{
    double OldGmin, gtarget, factor, startgmin;
    int converged;
...
static int
spice3_gmin_fast (CKTcircuit *ckt, GMRESarr *arr, long int firstmode,
             long int continuemode, int iterlim)
{
    int converged, i;
...
static int
gillespie_src_fast (CKTcircuit *ckt, GMRESarr *arr, long int firstmode,
               long int continuemode, int iterlim)
{
    int converged, i, iters;
    double ConvFact;
...
static int
spice3_src_fast (CKTcircuit *ckt, GMRESarr *arr, long int firstmode,
            long int continuemode, int iterlim)
{
    int converged, i;
